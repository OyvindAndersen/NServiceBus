namespace NServiceBus.Core.Tests.Routing
{
    ...
    public class UnicastSendRouterTests
    {
        ...
        public async Task When_routing_command_to_logical_endpoint_without_configured_instances_should_route_to_a_single_destination()
        {
            ...
            routingTable.AddOrReplaceRoutes(Guid.NewGuid(), new List<RouteTableEntry> {new RouteTableEntry(typeof(Command), UnicastRoute.CreateFromEndpointName(logicalEndpointName), RoutePriority.SpecificType) });
        ...
        {
            var sales = "Sales";
            routingTable.AddOrReplaceRoutes(Guid.NewGuid(), new List<RouteTableEntry> { new RouteTableEntry(typeof(Command), UnicastRoute.CreateFromEndpointName(sales), RoutePriority.SpecificType) });

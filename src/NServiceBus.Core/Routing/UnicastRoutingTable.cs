namespace NServiceBus.Routing
{
    ...
    public class UnicastRoutingTable
    {
        ...
        /// </summary>
        /// <param name="groupKey">Key for the route group.</param>
        /// <param name="entries">Group entries.</param>
        public void AddOrReplaceRoutes(object groupKey, IList<RouteTableEntry> entries)
        {
            lock (updateLock)
            {
                routeGroups[groupKey] = entries;
                ...
                foreach (var entry in routeGroups.Values.SelectMany(g => g))
                {
                    ...
                    if (!newRouteTableTemplate.TryGetValue(entry.MessageType, out existing) || entry.Priority.IsHigherThan(existing.Priority))
                    {
